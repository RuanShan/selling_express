...
RSpec.describe "mws_requests/new", type: :view do
  before(:each) do
    assign(:mws_request, MwsRequest.new(
      :amazon_request_id => "MyString",
      :request_type => "MyString"
    ))
  end

  it "renders new mws_request form" do
    render

    assert_select "form[action=?][method=?]", mws_requests_path, "post" do
